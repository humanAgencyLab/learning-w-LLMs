...
// Real shape: getCoursePerformanceSummary().quizByTopic — per-user
// FIRST-ATTEMPT pass rate by topic, the dashboard topic table's metric.
const PERF = {
  quizByTopic: [
    { title: 'Methods', firstAttemptPassRate: 63.2, studentsWithAttempts: 19 },
    { title: 'Variables and Data Types', firstAttemptPassRate: 57.9, studentsWithAttempts: 19 },
    { title: 'Logical Operators', firstAttemptPassRate: 67.1, studentsWithAttempts: 18 },
    { title: 'Number Systems', firstAttemptPassRate: 71.4, studentsWithAttempts: 18 },
    { title: 'Drafts Only', firstAttemptPassRate: null, studentsWithAttempts: 0 },
  ],
};
const AT_RISK = [
...
describe('computePinnedInsightFacts — deterministic ordered facts', () => {
  it('lead = true weakest topic by first-attempt pass rate, exact number', () => {
    const facts = computePinnedInsightFacts({ performance: PERF, atRisk: AT_RISK });
  ...
  it('is a pure function: same input, same output, every time', () => {
    const a = computePinnedInsightFacts({ performance: PERF, atRisk: AT_RISK });
    const b = computePinnedInsightFacts({ performance: PERF, atRisk: AT_RISK });
    expect(a).toEqual(b);
  });

  it('null-rate topics (no attempts) never rank', () => {
    const facts = computePinnedInsightFacts({ performance: PERF, atRisk: [] });
    expect(facts.every((f) => f.name !== 'Drafts Only')).toBe(true);
  });

  it('B3 guardrail: logs loudly if the lead ever IS Methods', () => {
    const spy = jest.spyOn(console, 'error').mockImplementation(() => {});
    computePinnedInsightFacts({ performance: { quizByTopic: [{ title: 'Methods', firstAttemptPassRate: 40, studentsWithAttempts: 10 }, { title: 'Loops', firstAttemptPassRate: 90, studentsWithAttempts: 10 }] }, atRisk: [] });
...
describe('runInsightCards pinned mode — model owns wording only', () => {
  ...
  it('valid model wording passes through; id/chartRef forced from the facts', async () => {
    ...
    const { insightCards } = await runInsightCards({ performance: PERF, atRisk: AT_RISK, pinnedFacts: FACTS });
  ...
  it('MODEL SWAP INVARIANCE: two totally different model outputs yield identical conclusions', async () => {
    ...
    const r1 = await runInsightCards({ performance: PERF, atRisk: AT_RISK, pinnedFacts: FACTS });
    const r2 = await runInsightCards({ performance: PERF, atRisk: AT_RISK, pinnedFacts: FACTS });
  ...
  it('non-pinned call (no pinnedFacts) keeps the original free-form path', async () => {
    ...
    const { insightCards } = await runInsightCards({ performance: PERF, atRisk: AT_RISK });
